...
contract Minter is IMinter {
    ...
    constructor(
        ...
        team = msg.sender;
        teamRate = 30; // 30 bps
        _kodo = IKodo(IVotingEscrow(__ve).token());
    ...
    function initialize(
        ...
        _kodo.mint(address(this), max);
        _kodo.approve(address(_ve), type(uint).max);
    ...
    function circulating_supply() public view returns (uint) {
        return _kodo.totalSupply() - _ve.totalSupply();
    }
    ...
    function update_period() external returns (uint) {
        ...
        if (block.timestamp >= _period + WEEK && initializer == address(0)) { // only trigger if new week
            ...
            uint _balanceOf = _kodo.balanceOf(address(this));
            if (_balanceOf < _required) {
                _kodo.mint(address(this), _required - _balanceOf);
            }

            require(_kodo.transfer(team, _teamEmissions));
            require(_kodo.transfer(address(_rewards_distributor), _growth));
            _rewards_distributor.checkpoint_token(); // checkpoint token balance that was just minted in rewards distributor
            _rewards_distributor.checkpoint_total_supply(); // checkpoint supply

            _kodo.approve(address(_voter), weekly);
